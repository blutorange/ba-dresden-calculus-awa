...
\begin{definition}{Tangentenäherung einer Funktion}{}
...
\begin{figure}
    ...
    \caption{Graphische Interpretation eines Gebietsintegrals in kartesischen Koordinaten}
...
Wir gehen davon aus, dass der Kreis seinen Mittelpunkt im Koordinatenursprung besitzt und den Radius $R$ aufweist. Unsere Funktion hängt nun nicht mehr von $x$ und $y$ ab, sondern vom Abstand zum Ursprung $r$ und von Winkel mit der Abszisse $alpha$.
...
\begin{definition}{Gebietsintegral in Polarkoordinaten Koordinaten}{AreaIntPolar}
...
Schließlich gibt es noch eine Regel für die Ableitung einer Umkehrfunktion:
...
\begin{statement}{Lokaler Extremwert einer einstelligen Funktion}{CondExSingleVar}
    Sei $n$ eine gerade Zahl. An der Stelle $x_0$ liegt eine lokale Extremstelle der einstelligen Funktion $f$ vor, wenn gilt:
    $$
        f^{(1)}(x_0) = f^{(2)}(x_0) = \dots = f^{(n-1)}(x_0) = 0, f^{(n)}(x_0) \ne 0
    $$
    Zudem handelt es sich bei der Extremstelle um
    \begin{itemize}
        \item ein Minimum, wenn $f^{(n)}(x_0) > 0$ gilt.
        \item ein Maximum, wenn $f^{(n)}(x_0) < 0$ gilt.
    \end{itemize}
\end{statement}
