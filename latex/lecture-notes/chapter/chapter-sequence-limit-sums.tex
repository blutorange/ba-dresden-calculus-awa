...
\begin{example}{Partialsummen der geometrischen Reihe}{PartSumGeoSer}
	Die geometrische Folge lautet $a_n = p \cdot q^n$ mit $p,q\in\R$. Um die Partialsummen $s_n = \sum\limits_{i=0}^n p\cdot q^i$ zu bestimmen, bedienen wir uns eines kleinen Umformungstricks. Die Partialsumme wird mit der konstanten $q$ multipliziert und von der ursprünglichen Partialsumme abgezogen, sodass sich fast alle Summanden wegheben:
...
Aus den obigen Beispielen können wir die folgende allgemeine Regel ableiten, die für den Grenzwert eines Quotienten von Polynomen gilt:
...
\begin{definition}{Grenzwert einer Funktion}{LimFun}
    Sei $f:\R\to\R$ eine Funktion und $A = \left\lbrace (a_n) | \lim\limits_{n\to\infty} a_n = x_0 \right\rbrace$ die Menge aller konvergenten Zahlenfolgen mit dem Grenzwert $x_0\in\R$. Falls es ein $\alpha\in\R$ gibt, sodass $\lim\limits_{n\to\infty} f(a_n) = \alpha$ für alle solchen Zahlenfolgen gilt, so heißt $\alpha$ der \textbf{Grenzwert von $f$ gegen $x_0$}.
...
\begin{example}{Grenzwertregeln für Grenzwert einer Funktion}{LimFunSumQuot}
    Auf $\lim\limits_{x_0\to 0} \frac{n^2+n+2}{n^3+1}$ lassen sich analog die Grenzwertregeln \ref{eq:LimSum} und \ref{eq:LimQuot} anwenden und man erhält: $\frac{0^2+0+2}{0^3+1} = 2$.
\end{example}
...
\begin{example}{Anwendung des Quotientenkriteriums}{RatioTest}
    Untersucht werden soll, ob $s_n = \sum\limits_{i=1}^\infty \frac{n!}{n^n}$ existiert. Das Wurzelkritierum liefert (Betragsstrichte sind weggelassen, da alle Glieder positiv):
    ...
    Letzeres Gleichheitszeichen folgt aus \ref{eq:LimitExp}. Da $2\le e \le 3$ gilt, können wir für $q$ die Abschätzung vornehmen: $\frac{1}{3} \le q \le \frac{1}{2}$. Damit haben wir gezeigt, dass $q < 1$, somit ist die Reihe $s_n$ konvergent.
\end{example}
