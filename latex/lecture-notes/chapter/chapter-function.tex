...
\begin{definition}{Begriff der Funktion}{Function}
    Eine \textbf{Funktion} $f$ ist eine eindeutige Abbildung beziehungsweise Zuordnung von Elementen einer Grundmenge $\mathcal{S}$ zu Elementen einer Bildmenge $\mathcal{C}$. Die Elemente der Grundmenge heißen \textbf{Argumente}, die Elemente der Bildmenge \textbf{Funktionswerte}. Eindeutigkeit meint hierbei, dass keinem Argument mehr als ein Funktionswert zugeordnet ist.

    Wird jedem Argument ein Funktionswert zugeordnet, spricht man von einer \textbf{totalen Funktion}. Besteht die Zuordnung nur für manche Argumente, spricht man von einer \textbf{partiellen Funktion}. Die Menge $\mathcal{D} \subseteq \mathcal{S}$, für die ein Funktionswert erklärt ist, heißt Definitionsbereich.

    Man schreibt: $f: \mathcal{S} \to \mathcal{C}$.
\end{definition}
...
\begin{example}{Definitionsbereich und Wertebereich}{DomainCodomain}
    Durch die Zuweisung $x\mapsto\frac{1}{x}$ ist eine Abbildung $f: \R \to \R$ zwischen reellen Zahlen definiert. Für die Zahl $0$ ist keine Zuordnung möglich, da hier der Ausdruck $1/0$ nicht erklärt ist. Es handelt sich also um eine partielle Funktion mit dem Definitionsbereich $\mathcal{D} = \lbrace x \in \R | x \ne 0 \rbrace = \R \setminus \lbrace 0 \rbrace$. Außer der $0$ kann jeder Funktionswert angenommen werden, mithin lautet der Wertebereich ebenfalls $\mathcal{C} = \R \setminus \lbrace 0 \rbrace$.
\end{example}
...
\begin{definition}{Identitätsfunktion}{IdFun}
    Sei $\mathcal{M}$ eine Menge. Die Funktion $id_\mathcal{M}: \mathcal{M} \to \mathcal{M}$ heißt \textbf{Identitätsfunktion} auf der Menge $\mathcal{M}$ und ist gegeben durch $id_\mathcal{M}: x \mapsto x$.
\end{definition}

Sie wird auch geschrieben als $\mathbbm{1}_\mathcal{M}$ geschrieben, oder, wenn die Menge $\mathcal{M}$ aus dem Kontext heraus bekannt ist, auch nur als $id$ beziehungsweise $\mathbbm{1}$.
...
\begin{definition}{Injektivität einer Funktion}{Injective}
    Sei $f: \mathcal{S} \to \mathcal{C}$ eine Funktion mit Grundmenge $\mathcal{S}$ und Zielmenge $\mathcal{C}$. $f$ heißt \textbf{injektiv}, wenn eine Funktion $g: \mathcal{C} \to \mathcal{S}$ existiert mit der Eigenschaft:
    $$
    g \circ f = \mathbbm{1}_S
    $$
    Äquivalent dazu ist die Forderung, dass kein Funktionswert doppelt angenommen wird, sich also gleiche Funktionswerte nur aus gleichen Argumenten ergeben: $\forall x_1,x_2\in\mathcal{S}: f(x_1)=f(x_2) \implies x_1=x_2$.
\end{definition}

\begin{definition}{Surjektivität einer Funktion}{Surjective}
    Sei $f: \mathcal{S} \to \mathcal{C}$ eine Funktion mit Grundmenge $\mathcal{S}$ und Zielmenge $\mathcal{C}$. $f$ heißt \textbf{surjektiv}, wenn eine Funktion $g: \mathcal{C} \to \mathcal{S}$ existiert mit der Eigenschaft:
    $$
    f \circ g = \mathbbm{1}_C
    $$
    Äquivalent dazu ist die Forderung, dass jeder Funktionswert wenigstens einem Argument zugeordnet ist, dass also $f(\mathcal{S}) = \mathcal{C}$ gilt. Dabei bezeichnet $f(\mathcal{S})$ die Menge aller Elemente, die man erhält wenn man auf jedes Argument die Funktion anwendet.
\end{definition}
...
\begin{statement}{Umkehrbarkeit einer Funktion}{InverseFun}
    Sei $f: \mathcal{S} \to \mathcal{C}$ eine Funktion mit Grundmenge $\mathcal{S}$ und Zielmenge $\mathcal{C}$. $f$ heißt \textbf{bijektiv}, wenn $f$ sowohl injektiv als auch surjektiv ist. Dann existiert genau eine Funktion $f^{-1}: \mathcal{C} \to \mathcal{S}$, welche die Bedingungen für Injektivität und Surjektivität erfüllt:
    $$
    f^{-1} \circ f = \mathbbm{1}_S, f \circ f^{-1} = \mathbbm{1}_C
    $$
...
\begin{statement}{Gerader und ungerader Anteil einer Funktion}{SplitEvenOddFun}
    Sei $f: \R \to \R$ eine einstellige reellwertige Funktion mit einem Definitionsbereich $\mathcal{D}$ derart, dass $x\in\mathcal{D} \iff -x\in\mathcal{D}$ gilt, also zu jedem Argument auch sein Negiertes zum Definitionsbereich gehört. Seien weiterhin $e$ und $o$ wie folgt definierte Funktionen:
...
\begin{definition}{Periode einer einstelligen Funktion}{PeriodUnivarFun}
    Sei $f: \R \to \R$ eine einstellige reellwertige Funktion mit Definitionsbereich $\mathcal{D}$. $f$ heißt \textbf{periodisch}, wenn es eine reelle Zahl $p>0$ derart gibt, dass gilt:
    $$
        \forall x \in \mathcal{D}: f(x) = f(x+p)
    $$
...
\begin{definition}{Stetigkeit einer reellwertigen Funktion}{ContFun}
    Sei $f: \R^n \to \R$ eine $n$-stellige, reellwertige Funktion mit Definitionsbereich $\mathcal{D}$. $f$ heißt \textbf{stetig} an der Stelle $x_0\in\mathcal{D}$, wenn der Grenzwert für diese Stelle existiert und dem dem Funktionswert gleich ist, also gilt:
    $$
...
\begin{definition}{Stetige Ergänzung einer Funktion}{ContinousContinuation}
    Sei $f: \R^n\to\R$ eine mehrstellige Funktion mit Definitionsbereich $\mathcal{D}\subseteq\R^n$ und $x_0\notin\mathcal{D}$ eine Stelle außerhalb des Definitionsbereichs (\textbf{Definitionslücke}). Falls der Grenzwert $\lim\limits_{x\to x_0} = \alpha$ existiert, heißt die Funktion $\bar{f}$, welche durch Hinzunahme des Grenzwert aus $f$ entsteht, die \textbf{stetige Ergänzung} von $f$:
    $$
        ...
        \end{cases}
    $$
    $\bar{f}$ hat somit nun den Definitionsbereich $\mathcal{D} \cup \lbrace x_0 \rbrace$.
\end{definition}
...
\begin{definition}{Besondere Stellen einer reellwertigen Funktion}{SpecValues}
    Sei $f: \R^n\to\R$ eine Funktion mit Definitionsbereich $\mathcal{D}$. Dann heißt
    \begin{itemize}
        \item $x_0\in\mathcal{D}$ eine \textbf{Nullstelle}, wenn gilt $f(x_0) = 0$.
        ...
        \item $x_0 \notin \mathcal{D}$ eine \textbf{Polstelle}, wenn der links- oder rechtsseitige Grenzwert bestimmt divergent ist.
    \end{itemize}
\end{definition}
...
\begin{definition}{Extremstelle einer reellwertigen Funktion}{ExtrFun}
    Sei $f: \R^n \to \R$ eine $n$-stellige reellwertige Funktion. Eine Stelle $x_0\in\mathcal{D}$ heißt ein \textbf{globales Maximum} (\textbf{globales Minimum}) von $f$ bezüglich einer Menge $M\subset\mathcal{D}$, falls der Funktionswert an dieser Stelle kleiner oder gleich (größer oder gleich) aller Funktionswerte in $M$ ist:
    $$
