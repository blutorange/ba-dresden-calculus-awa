...
\begin{definition}{Taylor-Reihe}{TaylorExpansion}
    Die durch eine Funktion $f$ an der \textbf{Entwicklungsstelle} $x_0$ erzeugte \textbf{Taylor-Reihe} vom \textbf{Grad n} $T_n[f]$ ist gegeben durch:
    $$
        f(x) = \underbrace{\sum\limits_{i=0}^n \frac{f^{(i)}(x_0)}{i!} (x-x_0)^i}_{\text{Taylor-Reihe }T_n[f](x)} + \underbrace{R_n(x)}_{\text{Restglied}}
    $$
...
\begin{definition}{Fourier-Reihe}{FourierExpansion}
    Die durch eine Funktion $f$ mit der Periode $T$ erzeugte \textbf{Fourier-Reihe} $F[f]$ ist gegeben durch:
    $$
        f(x) = \underbrace{\sum\limits_{i=0}^n a_i \cos(\frac{2\pi}{T}ix) + b_i \sin(\frac{2\pi}{T}ix)}_{\text{Fourier-Reihe } F[f](x)} + \underbrace{R_n(x)}_{\text{Restglied}}
    $$
